...
/***** by wxy   - wxy_hly ******/
document.write("<div id='copydiv'><button class='btn' onclick=\"scaleplus()\">Zoom In</button><button class='btn' onclick=\"scalemoin()\">Zoom Out</button> | <button class='btn' style='background:#F00;color:#FFF' onclick=\"closeTip()\" class='btn'>Close</button><b>　Copy RLE Code, paste into Golly directly</b><p><textarea onmouseover=\"this.select();\" id='copydivinner' style='width:90%;height:80px'></textarea></div>")
RLES = [];
function scaleplus() {
	var i = _RLES_i;
...
function onloadeddata() {
	setTimeout(function () {
		...
		for (var i = 0; i < RLES.length; i++) {
			...
			wrapper.className = 'rle-wrapper';
			RLES[i]["canvas"].parentNode.insertBefore(wrapper, RLES[i]["canvas"]);
			wrapper.appendChild(RLES[i]["canvas"]);
		}
	}, 500);
}
