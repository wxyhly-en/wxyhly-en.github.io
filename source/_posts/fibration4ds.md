...
很简单的一个式子。我们详细解释一下：先在四维空间中选任意一个平面$A$为坐标轴$z1$（这个轴是复数的，所以是二维的），再选平面$A$的绝对垂直（因为坐标系$z_1Oz_2$正交）平面$B$为坐标轴$z2$。比如我们就选$xy$和$zw$这对面吧，具体规定$(x,y,z,w)\to(z_1,z_2)$的计算方法（可以任意规定，这里只是图方便）：
$$z_1=x+iy $$$$ z_2=z+iw$$
