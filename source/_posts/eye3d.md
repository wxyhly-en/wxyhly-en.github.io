---
title: "The Four-Dimensional World (Part 5): Experiencing 4D Vision and Spatial Orientation"
tags:
  - 4dViewer
  - Series Articles
...
<div style="float:right">![Panel settings: blue for polytope geometry-related, red for rendering-related, see explanation for details](/img/eye3d004.png)</div>
...
- First select "Prism" in the shape selection dropdown
- Select "Torus" in the base cell dropdown under Prism
- An 8-sided torus appears in the view. Adjusting the U and V values in the Torus section changes the number of segments - more segments make the torus smoother but laggier.
- Since the toroidal prism is a curved-cell figure, there's no need to display edges that become abnormally messy with increasing segments - they consume rendering resources and cause lag. Select Display->Cells to hide them.
- If you find the blue background fog too heavy, expand the Render section and reduce the bgColor4Flow value; similarly, if the geometry seems too transparent, increase the Flow value (increasing Flow will increase opacity of both geometry and background together, so you need to correspondingly reduce bgColor4Flow). Of course, if these settings seem too complex, you can directly select the Cells (defog) option from the preset dropdown.
...
- More hotkeys can be <a href="/4dViewer/#渲染设置" target="_blank">found here</a>.

![Hypersphere rendering edges (using Hopf latitude-longitude grid) plus high stacking resolution lags into a PowerPoint](/img/eye3d006.jpg)
...
![What we need to do is pretend to "directly see" (just skillfully conditioned reflex) the entire blue box, then understand 4D on top of that](/img/eye3d003.png)
<a name="hh5"></a>
...
3. The lower right side cross-section is the $yz$ direction cross-section, where the horizontal axis is $z$ direction (lateral-front/lateral-back), and the vertical axis is the $y$ gravity direction. I need to go right, corresponding to lateral-front, so press `E` to move and align with the slope. You'll find that after aligning with the slope, the slope also appears in the lower left cross-section. Press `W` to advance and climb to the top.
...
2. I find there are upward slopes to the left, right, and lateral-front, but the lateral-front one is largest and should be closest, so I should press `E` to move lateral-front, making the truncated pyramid slanting lateral-front become a downward-facing regular truncated pyramid, then I can go up (don't understand? [See here](#align)). But I don't want to repeat the previous route - I specifically want to take the far one and go up from the left slope, so I press `A` to move left to align with the bottom of the slope.
...
5. After turning right, I find the truncated pyramid below isn't straight, leaning lateral-back, press `Q` to adjust;
...
11. A square thing is already exposed at the edge of the lateral-front view, so use the mouse to look that way, observing small patches of sky color - that's the sky at the other end of the cave. Note the road surface truncated pyramid must stay directly below, can't shift left/right or lateral-front/lateral-back to avoid falling;
12. Aim at the cave entrance and press `W` to enter the tunnel, while constantly adjusting direction with the mouse, keeping the cave entrance sky aligned with the crosshair (must align in both left/right and lateral-front/lateral-back directions), otherwise you'll hit the wall.
![Corresponding to the numbered steps above](/img/eye3d014.jpg)
<a name="align"></a>

Here I'll emphasize what counts as aligning the truncated pyramid - see the diagram below. Only the middle truncated pyramid in the diagram is aligned. If it leans left, it means you're deviating to the right side of the road, and vice versa. Note that left-right and lateral-front/lateral-back are essentially both lateral directions. If the view isn't cubic (like cylindrical or spherical), 4D people have no external reference to distinguish left-right from lateral-front/lateral-back (they can only use their body organ positions). So if left-right needs to be aligned, then lateral-front/lateral-back also needs to be. If you're uncomfortable looking at lateral-front/lateral-back directions, you can press `Z` `X` to rotate the view. This rotation exchanges left-right with lateral-front/lateral-back while keeping forward direction and vertical direction unchanged (this is the [self-rotation](/archives/trans4d/#selfchhe) we talked about when discussing 4D cars!).
